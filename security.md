...
Fedora-Mail-in-a-Box Security Guide
============================

Fedora-Mail-in-a-Box turns a fresh Fedora Server 64-bit machine into a mail server appliance by installing and configuring various components.

This page documents the security features of Fedora-Mail-in-a-Box. The term “box” is used below to mean a configured Fedora-Mail-in-a-Box.
...
The primary goal of Fedora-Mail-in-a-Box is to make deploying a good mail server easy, so we balance ― as everyone does ― privacy and security concerns with the practicality of actually deploying the system. That means we make certain assumptions about adversaries. We assume that adversaries . . .
...
Console access (e.g. via SSH) is configured by the system image used to create the box, typically from by a cloud virtual machine provider (e.g. Digital Ocean). Fedora-Mail-in-a-Box does not set any console access settings, although it will warn the administrator in the System Status Checks if password-based login is turned on.
...
The following services are protected: SSH, IMAP (dovecot), SMTP submission (postfix), webmail (roundcube), Nextcloud/CalDAV/CardDAV (over HTTP), and the Fedora-Mail-in-a-Box control panel & munin (over HTTP).
